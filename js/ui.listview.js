...
function init_view(view) {
    ...
    vcard.find('.vcard_follow').click(function (event) {
        ...
        if ($(this).hasClass('unfo')) {
            ...
            globals.twitterClient.destroy_list_subscriber(view.screen_name
                , view.slug, function () {
                toast.set(
                    _('unfollow_at') + view.screen_name + '/' + view.slug + ": " + _('successfully')).show();
            ...
            globals.twitterClient.create_list_subscriber(view.screen_name
                , view.slug, function () {
                toast.set(
                    _('follow_at') + view.screen_name +'/' + view.slug+ ": " + _('successfully')).show();
    ...
    vcard.find('.vcard_delete').click(function () {
        var ans = confirm(_('deleting_list_confirm'));
        if (ans) {
            toast.set(_('deleting_list_at') + view.screen_name + '/' + view.slug + " ...").show();
            globals.twitterClient.destroy_list(view.screen_name, view.slug
            , function () {
                toast.set(_('deleting_list_at') + view.screen_name + '/' + view.slug + _('successfully')).show();
...
function render_list_view(view, list_obj, proc) {
    ...
    ui.Slider.set_icon(view.name, list_obj.user.profile_image_url, ui.Slider.FLOAT_ICON); //4.0: Fix

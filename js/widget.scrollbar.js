(function() {
  var Scrollbar, root, _ref, tsX, tsY;
  ...
  Scrollbar = (function() {
    ...
    Scrollbar.prototype.on_touchstart = function(event) {
	tsX = event.originalEvent.touches[0].pageX;
	tsY = event.originalEvent.touches[0].pageY;	
    };    

    Scrollbar.prototype.on_touchmove = function(event) {
	var delta, offsetY;
	if (this.disabled) return;
	if (event.originalEvent.touches[0].pageY > 50 || event.originalEvent.touches[0].pageY < -50) {
		delta = (event.originalEvent.touches[0].pageY - tsY) / 5;
		offsetY = this.content.get(0).offsetTop - delta;
		this.scroll(offsetY);
		return false;
        } else {
    ...
    Scrollbar.prototype.on_touchend = function(event) {
        var delta;
	if (this.disabled) return;
	if (event.originalEvent.changedTouches[0].pageX !== tsX){
		delta = (event.originalEvent.changedTouches[0].pageX - tsX) / 5;
		if (delta < -50) {
    ...
    Scrollbar.prototype.bind = function() {
      ...
      this.content.on('MozMousePixelScroll', function(ev) {
        return _this.on_wheel(ev);
      });
      this.content.on('touchstart', function(ev) {
        return _this.on_touchstart(ev);
      });
      this.content.on('touchmove', function(ev) {
        return _this.on_touchmove(ev);
      });
      this.content.on('touchend', function(ev) {
        return _this.on_touchend(ev);
      });

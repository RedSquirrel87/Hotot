...
isMiniView: false, // 1-button mode
...
function init () {
    ...
    $('#indication').on('click', '.indicator_btn', function (ev) {
        ...
        } else if (view_name == 'close') {
            if (confirm(_('close_all_views_confirm'))) {
		for (var k in ui.Main.views) {
			ui.Main.views[k].destroy(ui.Main.views[k]);
		}
	    } 
        ...
        } else if (view_name == 'columns') { // 1-button mode
		ui.Header.closeAll();
                $('#indicator_menu_btn').addClass('hlight');
		$('#indicator_menu_slider').css({'right': ($('#indicator_menu_btn').offset().right)+'px'}).show();
		$('#indicator_menu_slider').empty();
		
		$('#indicator_menu_slider').append('<li><a href="#add">'+_('add_new_views')+'</a></li><hr/>');
		$('#indicator_menu_slider li:last a').click(function (event) {
			$('#indicator_menu_slider').hide();
			$('#slider_menu').css({'right': ($('#indicator_menu_btn').offset().right)+'px'}).show();
		...
		for (var k in ui.Main.views) {
			$('#indicator_menu_slider').append('<li><a href="#'+ui.Main.views[k].name+'">'+ui.Main.views[k].title+'</a></li>');
			$('#indicator_menu_slider li:last a').click(function(){
				var nview = $(this).attr('href').substring(1);
				ui.Slider.slide_to(nview);
				if (nview === 'search') {
					setTimeout(function () {
		...
		$('#indicator_menu_slider').append('<hr/><li><a href="#close">'+_('close_all_views')+'</a></li>');
		$('#indicator_menu_slider li:last a').click(function (event) {
			$('#btn_close_all_views').click();
		}); 
        } else {
    ...
    // 1-button mode
    $('#indicator_menu_slider').mouseleave(function (event) {
        $('#indicator_menu_btn').removeClass('hlight');
	$('#indicator_menu_slider').hide();
    });
    ...
    $('#slider_menu a').click(function(){
        ...
        switch (name) {
        case 'people':
            ret = widget.DialogManager.prompt(_('input_screenname'), 
                _('input_screenname_form'),
                function (ret) {
                    ret = $.trim(ret);
                    if (ret.length !== 0) {
        ...
        break;
        case 'list':
            ret = widget.DialogManager.prompt(_('input_listname'),
                _('input_listname_form'),
                function (ret) {
                    ret = $.trim(ret);
                    if (ret.length !== 0) {
                        if (ret[0] == '@') ret = ret.substring(1);
                        open_list(
				ret.substring(0, ret.indexOf('/')),
        ...
        break;
	case 'allmylist':	
		toast.set(_('opening_all_lists')).show()	
		ret = globals.twitterClient.get_user_lists(globals.myself.screen_name,"",
		function (ret) {
			var str = '', owner = '', slug = '', tm = 800;
			for (i = 0; i < ret.length; i++) {
				str = ret[i]['uri'].substring(1);
				owner = str.substring(0, str.indexOf('/'));
				slug = str.substring(str.lastIndexOf('/')+1);
				open_list(owner, slug, {}, true, tm*i);
			}
			toast.hide();
                });
        break;
...
function save_state() {
	if (ui.Slider.state !== null) {
		...
		if (conf.get_current_profile().preferences.sync_cols) {
			if (chrome && chrome.storage && chrome.storage.sync) {
				var key = conf.get_current_profile().preferences.access_token.user_id;
				var value = conf.get_current_profile().preferences.slider_state;
				var json = {};
				json[key] = value;
...
function resume_state() {
	if (conf.get_current_profile().preferences.sync_cols) {
		if (chrome && chrome.storage && chrome.storage.sync) {
			var key = conf.get_current_profile().preferences.access_token.user_id;
			var value = {order: [], views: {}};
			var json = {};
			json[key] = value;
			chrome.storage.sync.get(json, function(data) {
				hotot_log("SYNC","Download");
				if (data[key]) {
					conf.get_current_profile().preferences.slider_state = data[key];
				}
				ui.Slider.resume_state_old();
				daemon.update_all();
			});
		}
	} else {
...
// 4.0: Sync opened columns
resume_state_old:
function resume_state_old() { 
    ui.Slider.isStartup = true;
    ...
    if (ui.Slider.state == null) {
        ...
        for (var i = 0; i < ui.Slider.state.order.length; i += 1) {
            ...
            if (ui.Slider.state.views.hasOwnProperty(name)) {
                ...
                break;
                case 'people':
                    open_people(opts.screen_name, opts, true, true);
                break;
                case 'list':
                ...
                break;
		case 'replies':
		    open_replies(opts.query, opts.screen_name, opts, true);
		break;
                }
            }
        }
...
function add_view(name, opts) {
    if (name == 'search') {
        ...
        if ($('#search_tweetview').length == 0) {
            ...
            $(ui.Template.form_view(name, opts.title, 'tweetview')).insertBefore($('#search_tweetview'));
        }
    }
...
set_title_and_icon:
function set_title_and_icon(name, title, icon, style) {
    var btn = $('#indication .indicator_btn[href="#'+name+'"]');
...
function openViewSettingPanel(view, btn) {
	...
	if (view.hasOwnProperty(key)) {
		...
		if (conf.get_current_profile().preferences[key] === true) {
			$(n).attr('checked',true).prop('checked', true);
		} else {
			$(n).attr('checked',false).prop('checked', false);
		}
...
function addDefaultView(name, opts) {
    ...
    switch (name) {
    case 'trends':
    ui.Slider.add('trends'
        , {title: "Trending Topics", icon:'image/ic_trend.png'}
        , $.extend({ 'type':'tweet', 'title': "Trending Topics"
	    , 'load': ui.TrendingTopicsView.load_tt
            , 'load_success':  ui.TrendingTopicsView.load_tt_success
            , 'load_fail': null
            , 'loadmore': ui.TrendingTopicsView.load_tt
            , 'loadmore_success': ui.TrendingTopicsView.load_tt_success
            , 'loadmore_fail': null
    ...
    ui.Slider.add('home'
        ...
        , $.extend({ 'type':'tweet', 'title': _('home_timeline')
            ...
            , 'load_success': ui.HomeTabs.load_tweet_success // 4.0: Media-only filter
            , 'load_fail': null
            , 'loadmore_success': ui.HomeTabs.loadmore_tweet_success // 4.0: Media-only filter
            , 'loadmore_fail': null
            , 'former': ui.Template.form_tweet
            , 'destroy': ui.Main.destroy_view            
            , 'header_html': ui.Template.home_column_header_t()  // 3.6: Hide every RT in home
	    , 'init': ui.HomeTabs.init_view  // 4.0: Media-only filter
            , 'method': 'push'
            , 'interval': 120
            , 'item_type': 'id'
            , 'use_notify': false
	    , 'hide_rt': false  // 3.6: Hide every RT in home
        }, opts));
    break;
    case 'mentions':
    ...
    ui.Slider.add('messages'
        ...
        , $.extend({ 'type':'tweet', 'title': _('messages')
            , 'load': (conf.get_current_profile().preferences.use_new_dm_view_mode ? ui.Main.load_messages_new : ui.Main.load_messages)
            , 'loadmore': (conf.get_current_profile().preferences.use_new_dm_view_mode ? null : ui.Main.loadmore_messages) //2.2: New DM view mode
            , 'load_success': (conf.get_current_profile().preferences.use_new_dm_view_mode ? ui.Main.load_messages_success : ui.Main.load_tweet_success) //2.2: New DM view mode
            , 'load_fail': null
            , 'loadmore_success': (conf.get_current_profile().preferences.use_new_dm_view_mode ? null : ui.Main.loadmore_tweet_success) //2.2: New DM view mode
            , 'loadmore_fail': null
            , 'former': ui.Template.form_dm
	    , 'init': (conf.get_current_profile().preferences.use_new_dm_view_mode ? ui.DMTabs.init_view : null) //2.2: New DM view mode
            , 'destroy': ui.DMTabs.destroy_view // 4.0: Optimizations
            , 'header_html': ui.Template.common_column_header_t()
	    , 'header_html_ex': (conf.get_current_profile().preferences.use_new_dm_view_mode ? ui.Template.message_header_t : "") //2.2: New DM view mode
            , 'method': 'push'
            , 'interval': 120
            , 'item_type': 'id'
            , 'use_notify': true
    ...
    ui.Slider.add('retweets'
        ...
        , $.extend({ 'type':'tweet', 'title': _('retweets')
            , 'load': ui.Main.load_retweets 
            , 'loadmore': ui.Main.loadmore_retweets
            , 'load_success': ui.Main.load_tweet_success
            , 'load_fail': null
            , 'loadmore_success': ui.Main.loadmore_tweet_success
            , 'loadmore_fail': null
    ...
    ui.Slider.add('favs'
        , {title: (_('likes')), 
	icon: ('image/ic_like.png') }
        , $.extend({ 'type':'tweet', 'title': (_('my_likes'))

...
function init_view(view) {
    ...
    toggle.find('.people_view_fav_btn').text(_('likes'));
    ...
    function (event) {
        ...
        if ($(this).hasClass('unfo')) {
            ...
            function () {
                toast.set(
                    _("unfollow_at")+ view.screen_name+": "+ _("successfully")).show();
            ...
            function () {
                toast.set(
                    _("follow_at")+ view.screen_name+": "+ _("successfully")).show();
    ...
    // 3.6 Fix for blocked users
    vcard.find('.vcard_block').click('click',
    function (event) {
        var _this = this;
        if ($(this).hasClass('unblock')) {
		toast.set(_('unblock') + " @" + view.screen_name + "...").show();
		globals.twitterClient.destroy_blocks(view.screen_name,
		function (result) {
			toast.set(_('unblock') + " @"+ result.screen_name+": "+ _("successfully")).show();
			var pos = globals.blocking_ids.indexOf(result.id_str);
			if (pos !== -1) {
		                globals.blocking_ids.splice(pos, 1);
			}
			$(_this).text(_("block")).removeClass('unblock');
		});
		people_action_more_memu.hide();
        } else {
		if (!confirm(_('block_confirm') + " @"+view.screen_name+"?!\n")) return;
		toast.set(_('block') + " @" + view.screen_name + "...").show();
		globals.twitterClient.create_blocks(view.screen_name,
		function (result) {
			toast.set(_('block') +  " @"+ result.screen_name+": "+ _("successfully")).show();
			globals.blocking_ids.push(result.id_str);
			$(_this).text(_("unblock")).addClass('unblock');
		});
		people_action_more_memu.hide();
        }
    });    
    ...
    vcard.find('.mute_menu_item').click(
    function (event) {
        if (!confirm(_('mute_twitter_confirm') + " @"+view.screen_name+"?!\n")) return;
        toast.set(_('mute') + " @" + view.screen_name + "...").show();
        globals.twitterClient.mutes_users_create(view.screen_name,
        function (result) {
            toast.set(
                _('mute') +  " @"+ result.screen_name+": "+ _("successfully")).show();
            globals.mutes_ids.push(result.id);
        });
        people_action_more_memu.hide();
	view._header.find('.mute_menu_item').hide();
	view._header.find('.unmute_menu_item').show();
    });
    vcard.find('.unmute_menu_item').click(
    function (event) {
        toast.set(_('unmute') + " @" + view.screen_name + "...").show();
        globals.twitterClient.mutes_users_destroy(view.screen_name,
        function (result) {
            toast.set(
                _('unmute') + " @"+ result.screen_name+": "+ _("successfully")).show();
            var pos = globals.mutes_ids.indexOf(result.id);
            if (pos !== -1) {
                globals.mutes_ids.splice(pos, 1);
            }
        });
        people_action_more_memu.hide();
	view._header.find('.mute_menu_item').show();
	view._header.find('.unmute_menu_item').hide();
    });
    
    //3.6: No Retweets improvements
    vcard.find('.no_rt_menu_item').click(
    function (event) {
        if (!confirm(_('no_rt_twitter_confirm') + " @"+view.screen_name+"?!\n")) return;
        toast.set(_('no_rt') + " @" + view.screen_name + "...").show();
        globals.twitterClient.friendships_update(view.screen_name,false,
        function (result) {
            toast.set(
                _('no_rt') +  " @"+ result.relationship.target.screen_name +": "+ _("successfully")).show();
            globals.no_retweets_ids.push(result.relationship.target.id_str);
        });
        people_action_more_memu.hide();
	view._header.find('.no_rt_menu_item').hide();
	view._header.find('.yes_rt_menu_item').show();
    });
    vcard.find('.yes_rt_menu_item').click(
    function (event) {
        toast.set(_('yes_rt') + " @" + view.screen_name + "...").show();
        globals.twitterClient.friendships_update(view.screen_name,true,
        function (result) {
            toast.set(
                _('yes_rt') + " @"+ result.relationship.target.screen_name +": "+ _("successfully")).show();
            var pos = globals.no_retweets_ids.indexOf(result.relationship.target.id_str);
            if (pos !== -1) {
                globals.no_retweets_ids.splice(pos, 1);
            }
        });
        people_action_more_memu.hide();
	view._header.find('.no_rt_menu_item').show();
	view._header.find('.yes_rt_menu_item').hide();
    });    

    vcard.find('.report_spam_menu_item').click(
    function (event) {
        if(!confirm(_('report_spam_confirm'))) 
            return;
        toast.set(_('report_spam') + " @" + view.screen_name + "...").show();
        globals.twitterClient.create_blocks(view.screen_name,
        function () {
            toast.set(
                _('report_spam') + " @"+ view.screen_name+": "+ _("successfully")).show();
        });
        people_action_more_memu.hide();
    });
...
function switch_sub_view(view, name) {
    ...
    case 'owntweet':
        ...
        view.since_id = 1;
        view.former = ui.Template.form_tweet;
        view._load = ui.PeopleView.load_own_tweets;
        view._loadmore = ui.PeopleView.loadmore_own_tweets;
        view._load_success = ui.Main.load_tweet_success;
        view._loadmore_success = ui.Main.loadmore_tweet_success;
    break;
    case 'fav':
...
function render_people_view(self, user_obj, proc) {
    // 3.6
    var btn_report_spam = self._header.find('.report_spam_menu_item');
    ...
    //3.6
    ui.PeopleView.update_buttons(self,user_obj);
    //Fix for already opened tabs
    //To Do better: wait for sync
    setTimeout(function() {ui.PeopleView.update_buttons(self,user_obj)}, 3000);
...
//3.6
update_buttons:
function update_buttons(view,user_obj) {
    // 3.6: Fix for blocked users
    var btn_block = view._header.find('.vcard_block');
    
    //3.6: Support for mutes users
    var btn_mute = view._header.find('.mute_menu_item');
    var btn_unmute = view._header.find('.unmute_menu_item');
    
    //3.6: No Retweets improvements
    var btn_no_rt = view._header.find('.no_rt_menu_item');
    var btn_yes_rt = view._header.find('.yes_rt_menu_item');
    ...
    //3.6: Fix for blocked users
    if (globals.blocking_ids.indexOf(user_obj.id_str) !== -1) {
	// Blocked user
	btn_block.text(_('unblock'));
	btn_block.addClass('unblock');
    } else {
	btn_block.text(_('block'));
	btn_block.removeClass('unblock');    
    }
    
    //3.6: Support for mutes users
    if (globals.mutes_ids.indexOf(user_obj.id) !== -1) {
	// Muted user
	btn_mute.hide();
	btn_unmute.show();
    } else {
    ...
    //3.6: No Retweets improvements
    if (globals.no_retweets_ids.indexOf(user_obj.id_str) !== -1) {
	// No RT user
	btn_no_rt.hide();
	btn_yes_rt.show();
    } else {
...
function load_timeline_full(view, success, fail) {
        ...
        ui.PeopleView.render_people_view(view, user_obj 
            , function () {
                globals.twitterClient.get_user_timeline(null, view.screen_name
                    , view.since_id, null, conf.get_current_profile().preferences.items_per_request, success);
            });
    ...
    globals.twitterClient.show_user(view.screen_name, render_proc,
        function (xhr, textStatus, errorThrown) {
            if (xhr.status == 404) {
		widget.DialogManager.alert(_('person_not_exist')
                    , '@' + view.screen_name + ' ' + _('person_not_exist_extra') );		    
                view.destroy();
            }
    });
},
...
function load_timeline(view, success, fail) {
    globals.twitterClient.get_user_timeline(null, view.screen_name, view.since_id, null, conf.get_current_profile().preferences.items_per_request, success);
},
...
function loadmore_people(self, success, fail) {
    ...
    globals.twitterClient.get_user_timeline(null, self.screen_name, null, self.max_id, conf.get_current_profile().preferences.more_items_per_request, success);
},

load_own_tweets:
function load_own_tweets(view, success, fail) {
    globals.twitterClient.get_user_own_tweets(null, view.screen_name, view.since_id, null, conf.get_current_profile().preferences.items_per_request, success);
},

loadmore_own_tweets:
function loadmore_own_tweets(self, success, fail) {
    if (self.protected_user) {
        self._footer.hide();
        return;
    }
    globals.twitterClient.get_user_own_tweets(null, self.screen_name, null , self.max_id, conf.get_current_profile().preferences.more_items_per_request, success);
},

...
function WidgetAutoComplete(obj) {
    ...
    self.onKeyDown = function onKeyDown(event) {
        ...
        if (key_code == 38 || key_code == 40) { 
            ...
            self.candidate.stop().animate({scrollTop: target.get(0).offsetTop},0); 
	...
	var atIdx2 = rearText.lastIndexOf('@');	
	if (atIdx > atIdx2) {
		if (conf.get_current_profile().preferences.tag_autocomplete == false) {
			self.stopDetecting();
			return;
		} else {
			self.usermode = false;
		}
	} else if (atIdx2 > atIdx) {
		if (conf.get_current_profile().preferences.user_autocomplete == false) {
			self.stopDetecting();
			return;
		} else {
			self.usermode = true;
	...
	handleResult(self.quickFilter(self.inputText, (self.usermode ? globals.conversant : globals.hashtags)));
    };
    ...
    self.quickFilter = function quickFilter(text,type) {
        var result_list = type.filter(
            function (x) {
